...
| --- | --- | --- |
| Shared preset | Current | `default.json` is the public Renovate preset consumed by repositories through `github>MooseGooseConsulting/renovate-config`; it extends Renovate's `config:best-practices`. |
...
- Revisit release-age behavior after sampling consumers: `config:best-practices`
  currently supplies the npm security release-age window, while broader
  datasource delays should require explicit evidence.
