...
- `package.json`: maintenance workflow surface. `docs:update` regenerates the local Renovate docs snapshot, `renovate:validate` runs the repo-standard Renovate validation over `default.json` and `org-inherited-config.json`, and `verify` chains both.
